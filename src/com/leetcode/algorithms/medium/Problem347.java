...
public class Problem347 {
    ...
    public int[] topKFrequent(int[] nums, int k) {
        ...
        Arrays.stream(nums).forEach(num -> countMap.put(num, countMap.containsKey(num) ? countMap.get(num) + 1 : 1));

        PriorityQueue<Node> priorityQueue =
                new PriorityQueue<>(Comparator.comparing(Node::getNegativeFrequencyForDescendingSort));
        countMap.entrySet().forEach(entry -> priorityQueue.add(new Node(entry.getKey(), entry.getValue())));

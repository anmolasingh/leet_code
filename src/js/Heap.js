/** Javascript Implementation of a Heap */

/**
 * In order to get the position of a left child of a node, 
 * we multiply the index by 2, and add 1 (2n + 1).
 */
...
/**
 * In order to get the parent of a node, we subtract our index by 1, and divide by 2. 
 * We round down any floating numbers we get from dividing an odd number ((n - 1) / 2).
 */
...
MaxHeap.prototype.insert = function(element) {
  ...
  while (index !== 0 && this.comparator(this.heap[index], this.heap[parent(index)]) > 0) {
...
var myHeap = new MaxHeap((element1, element2) => {
	return element1.num - element2.num;
});

myHeap.insert({ num: 10 });
myHeap.insert({ num: 20 });
myHeap.insert({ num: 5 });